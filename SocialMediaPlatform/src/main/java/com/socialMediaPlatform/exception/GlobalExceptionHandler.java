...
public class GlobalExceptionHandler {
	...
	/**
	 * @param postException : it handles all types of PostException which is checked exception.
	 * @param req : general webRequest metadata.
	 * @return : responseEntity of customized error details with http status of error code 400.
	 */
	@ExceptionHandler(PostException.class)
	public ResponseEntity<MyErrorDetails> postExceptionHandler(PostException postException, WebRequest req) {
		MyErrorDetails err = new MyErrorDetails();
		err.setTimestamp(LocalDateTime.now());
